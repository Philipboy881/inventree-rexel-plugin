...
username = os.getenv("gebruiker")
password = os.getenv("wachtwoord")

# Maak een argumentparser
parser = argparse.ArgumentParser(description="Voer de productcode in om de info op te halen")

# Voeg een argument toe voor de productcode
parser.add_argument('productcode', type=str, help='De productcode waarvan je de onformatie wilt ophalen')

# Parse de argumenten
args = parser.parse_args()

# Haal de productcode op uit de commandoregelargumenten
productcode = args.productcode

# Print de productcode (optioneel, voor debugging)
#print(f"Productcode ingevoerd: {productcode}")

# Gebruikersnaam en wachtwoord en url's
...
#login functie
def Login(sesion, URL, username, password):
    ...
    # Haal de cookies op uit de sessie
...
def Get_Data_from_html(html, price, sku):
    ...
    algemene_informatie_1 = extract_table_data(tabel1)
    algemene_informatie_2 = extract_table_data(tabel2)
...
#verkrijg url en sku
product_url_sku = GET_product_url(session, productcode, searchbox_url)
#print (product_url_sku)

#verkijg de user prijs
price = GET_Price(session, price_url + product_url_sku['code'] + price_url1)
#print("Price", price)
